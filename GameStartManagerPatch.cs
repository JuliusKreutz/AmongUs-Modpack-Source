...
namespace Modpack
{
    ...
    {
        public static Dictionary<int, Tuple<byte, byte, byte>> playerVersions =
                ...
                if (AmongUsClient.Instance.HHBLOCGKFAB)
                {
                    ...
                    foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients)
                    {
                        ...
                        else if (!playerVersions.ContainsKey(client.Id))
                        {
                            blockStart = true;
                            message +=
                                $"<color=#FF0000FF>{client.Character.PPMOEEPBHJO.PCLLABJCIPC} has an outdated or no version of The Other Roles\n</color>";
                        }
                                 ...
                                 playerVersions[client.Id].Item3 != ModpackPlugin.Patch)
                        {
                            blockStart = true;
                            message +=
                                $"<color=#FF0000FF>{client.Character.PPMOEEPBHJO.PCLLABJCIPC} has an outdated version (v{playerVersions[client.Id].Item1}.{playerVersions[client.Id].Item2}.{playerVersions[client.Id].Item3}) of The Other Roles\n</color>";
                        }
                    }
