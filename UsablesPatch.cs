...
namespace Modpack
{
    ...
    {
        public static bool Prefix(Vent __instance, ref float __result, [HarmonyArgument(0)] GameData.LGBOMGHJELL pc,
            [HarmonyArgument(1)] out bool canUse, [HarmonyArgument(2)] out bool couldUse)
        {
            ...
            else
            {
                ...
                {
                    if (Sidekick.canUseVents && Sidekick.sidekick != null && Sidekick.sidekick == @object)
                        roleCouldUse = true;
                    else
                    {
            ...
            if (canUse)
            {
                ...
                canUse &= (num <= usableDistance &&
                           !PhysicsHelpers.DOAHONIIFJD(truePosition, position, Constants.KOMPKLHCHJI, false));
            }
    ...
    class UseButtonSetTargetPatch
    {
        static void Postfix(UseButtonManager __instance)
        {
            ...
            if (__instance.LHAKKAAOLLM != null && Trickster.trickster != null &&
            ...
            if (__instance.LHAKKAAOLLM == null && (blockSabotageJanitor || blockSabotageMafioso))
            {
                __instance.UseButton.sprite =
                    DestroyableSingleton<TranslationController>.CHNDKKBEIDG.GetImage(ImageNames.UseButton);
        ...
        static void Postfix(VitalsMinigame __instance)
        {
            if (__instance.PMKNJGLKGDD.Length > 10)
            {
                for (int i = 0; i < __instance.PMKNJGLKGDD.Length; i++)
                {
                    ...
                    vitalsPanel.Text.text = player.PCLLABJCIPC.Length >= 4
                        ? player.PCLLABJCIPC.Substring(0, 4).ToUpper()
                        : player.PCLLABJCIPC.ToUpper();
                }
            }
        }
    }
    ...
    class VitalsMinigameUpdatePatch
    {
        static void Postfix(VitalsMinigame __instance)
        {
            ...
            for (int k = 0; k < __instance.PMKNJGLKGDD.Length; k++)
            {
                ...
                {
                    DeadPlayer deadPlayer = deadPlayers?.Where(x => x.player?.PlayerId == player?.FNPNJHNKEBK)
                        ?.FirstOrDefault();
                    if (deadPlayer != null && deadPlayer.timeOfDeath != null)
                    {
                        ...
                        if (showHackerInfo)
                            ...
                            vitalsPanel.Text.text =
                                DestroyableSingleton<TranslationController>.CHNDKKBEIDG.GetString(
                                    Palette.MBDDHJCCLBP[(int) player.IMMNCAGJJJC],
            ...
            static bool Prefix(MapCountOverlay __instance)
            {
                ...
                for (int i = 0; i < __instance.CountAreas.Length; i++)
                {
                    ...
                    if (!commsActive)
                    {
                        ...
                        {
                            int num = plainShipRoom.roomArea.OverlapCollider(__instance.CMCPAKBKLDP,
                                __instance.HMNBGCJAGLM);
                            int num2 = num;
                            for (int j = 0; j < num; j++)
                            {
                                ...
                                if (!(collider2D.tag == "DeadBody"))
                                {
                                    ...
                                    if (!component || component.PPMOEEPBHJO == null ||
                                        component.PPMOEEPBHJO.MFFAGDHDHLO || component.PPMOEEPBHJO.IAGJEKLJCCI)
                                    {
                                        num2--;
                                    }
                                    else if (component?.KJAENOGGEOK?.material != null)
                                    {
                                        ...
                                        if (Hacker.onlyColorType)
                                        {
                                            ...
                                            color = Helpers.isLighterColor((byte) id)
                                                ? Palette.AEDCMKGJKAG[7]
                                                : Palette.AEDCMKGJKAG[6];
                                        }
                                ...
                                else
                                {
                                    ...
                                    if (component)
                                    {
                                        ...
                                        if (LGBOMGHJELL != null)
                                        {
                                            ...
                                            if (Hacker.onlyColorType)
                                                color = Helpers.isLighterColor(LGBOMGHJELL.IMMNCAGJJJC)
                                                    ? Palette.AEDCMKGJKAG[7]
                                                    : Palette.AEDCMKGJKAG[6];
                                            roomColors.Add(color);
                                        }
